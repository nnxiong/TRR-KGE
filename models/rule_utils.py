...
def filter_rules(rules_dict, min_conf, min_body_supp):
    new_rules_dict = dict()
    for k in rules_dict:
        new_rules_dict[k] = []
...
def filter_rules_hop(rules_dict, hop):
    new_rules_dict = dict()
    for k in rules_dict:
        new_rules_dict[k] = []
        for rule in rules_dict[k]:
            if hop == 1:
                cond = (
                    (rule["rule_type"] == "Symmetry" or rule["rule_type"] == "Inverse" or rule["rule_type"] == "Equivalent")
                )
            elif hop == 2:
                cond = (
                    (rule["rule_type"] == "Transitive2")
                )
            elif hop == 3:
                cond = (
                    (rule["rule_type"] == "Transitive3")
                )
            elif hop == 4:
                cond = (
                    (rule["rule_type"] == "Transitive3")
...
# def get_window_edges(all_data, test_query_ts, window=0):
#     if window > 0:
...
# def get_candidates(rule, rule_walks, test_query_ts, cands_dict, score_func, args, dicts_idx):
...
# def save_candidates(rules_file, dir_path, all_candidates, window, score_func_str, data):
...
#     filename = "{0}_cands_w{1}_{2}_{3}.json".format(
#         rules_file[:-11], window, score_func_str, data
#     )
