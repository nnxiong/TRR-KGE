...
def filter_rules(rules_dict, min_conf, min_body_supp, rule_type='all'):
    rule_type = ["Chain2", "Chain3", "Inverse", "Equivalent", "Transitive", "Symmetric", "Reflexive"] if rule_type == 'all' else [rule_type]
    new_rules_dict = dict()
    for k in rules_dict:
        new_rules_dict[k] = []
        for rule in rules_dict[k]:
            cond = (
                ...
                and (rule["rule_type"] in rule_type)
            )
            if cond:
